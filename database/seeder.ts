...
export const main = async () => {
  const habitsData = [
    ...
    { name: "Dormir 8h", weekdays: [0, 1, 2, 3, 4, 5, 6] } // Todos os dias
  ]
  ...
  const insertedHabits = await db
    .insert(habits)
    .values(habitsData.map((habit) => ({ name: habit.name, createdAt: creationDate.format("YYYY-MM-DD HH:mm:ss") })))
    .returning()
  ...
  // Gerar os dias completos (dayHabits) aleatoriamente desde 01/01/2025 até hoje
  const today = dayjs()
